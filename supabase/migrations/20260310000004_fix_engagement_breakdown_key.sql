-- =============================================================================
-- Fix: engagement_metrics breakdown was using wrong JSON key
-- =============================================================================
-- The engagement tracking SDK sends { id: "17a", name: "Card Flip" } but the
-- SQL was reading extra_data->>'type' (always NULL). This caused all engagements
-- to be grouped under a single 'interaction' key, duplicating Total Engagements.
--
-- Fix: Use extra_data->>'name' (sent by updated renderer) with fallback to
-- extra_data->>'id' (for historical events) and 'other' as final fallback.
-- =============================================================================
...
BEGIN
  ...
  eng_breakdown AS (
    SELECT
      t.d, t.cid, t.camp,
      jsonb_object_agg(t.eng_label, t.cnt) AS eng_metrics
    FROM (
      SELECT d, cid, camp,
        COALESCE(extra_data->>'name', extra_data->>'id', 'other') AS eng_label,
        COUNT(*) AS cnt
      FROM base WHERE event_type = 'engagement'
      GROUP BY d, cid, camp, COALESCE(extra_data->>'name', extra_data->>'id', 'other')
    ) t
  ...
  SELECT
    m.d, m.cid, m.camp,
    m.imp_served, m.imp_viewable, m.clicks, m.engs, m.vplays, m.vcompletes,
    m.dwell_ms,
    COALESCE(e.eng_metrics, '{}'::jsonb)
  FROM main_agg m
  LEFT JOIN eng_breakdown e
    ON e.d = m.d AND e.cid = m.cid
