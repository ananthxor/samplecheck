-- Fix: fetch_hourly_metrics and fetch_device_breakdown were querying ad_events
-- without joining to creatives. This caused deleted creatives' events to still
-- appear in hourly/platform charts while being absent from all other charts
-- (which use INNER JOIN creatives). Adding the same join makes all charts
-- show a consistent dataset.

-- ---------------------------------------------------------------------------
-- fetch_hourly_metrics — add INNER JOIN creatives for consistency
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- fetch_device_breakdown — add INNER JOIN creatives for consistency
-- ---------------------------------------------------------------------------
