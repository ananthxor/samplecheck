-- Populate engagement_metrics JSONB with per-type breakdown (e.g. {"flip": 10, "hover": 5}).
-- Previously returned '{}'::jsonb. Uses a correlated subquery to keep the flat SELECT structure
-- intact (no CTE restructure needed — minimises risk of breaking the working function).
...
BEGIN
  ...
  eng_breakdown AS (
    ...
    FROM (
      SELECT d, cid, camp, extra_data->>'type' AS etype, COUNT(*) AS cnt
